...
class CtrlVinho():
    ...
    def exibeTipo(self,event):
        tipo = self.limiteCons.escolhaTipo.get()
        self.limiteCons.textVinhos.config(state=tk.NORMAL)
        self.limiteCons.textVinhos.delete(1.0, tk.END)
        for vinho in self.listaVinhos:
            if vinho.tipo == tipo:
                self.limiteCons.textVinhos.insert(tk.END, vinho.getVinho() + "\n\n")
        self.limiteCons.textVinhos.config(state=tk.DISABLED)

    def exibeVariedade(self, event):
        variedade = self.limiteCons.escolhaVariedade.get()
        self.limiteCons.textVinhos.config(state=tk.NORMAL)
        self.limiteCons.textVinhos.delete(1.0, tk.END)
        for vinho in self.listaVinhos:
            if vinho.variedade == variedade:
                self.limiteCons.textVinhos.insert(tk.END, vinho.getVinho() + "\n\n")
        self.limiteCons.textVinhos.config(state=tk.DISABLED)